...
After building, you can test the library by running your Kotlin application:

```bash
./gradlew run
```

Or if you're building a JAR:
...
The application should automatically detect your platform and load the appropriate native library.

## Contributing

When contributing native library improvements:

1. Build and test on your platform
2. Update this documentation if adding new platforms or requirements
3. Include build artifacts for all supported platforms in your PR (if possible)
4. Add platform-specific notes if there are any special considerations
...
For automated builds, consider using GitHub Actions with multiple runners:
...
This ensures libraries are built for all platforms on each commit.
